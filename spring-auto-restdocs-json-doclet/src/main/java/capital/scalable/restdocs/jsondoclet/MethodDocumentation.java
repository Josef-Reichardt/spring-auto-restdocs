...
public class MethodDocumentation {
	...
	public static MethodDocumentation fromMethodDoc(DocletEnvironment docEnv,
		...
		md.comment = docEnv.getElementUtils().getDocComment(methodElement);

		docEnv.getDocTrees().getDocCommentTree(methodElement).getBlockTags()
				.forEach(tag -> {
					if (tag.getKind().equals(DocTree.Kind.PARAM)) {
						...
						md.tags.put(cleanupTagName(((BlockTagTree) tag).getTagName()),
								tag.toString());
					}
				});
